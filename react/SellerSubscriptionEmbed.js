...
const SellerSubscriptionEmbed = () => {
  ...
  const [mountingElement, setMountingElement] = useState(false)
  ...
  const handleContinue = async () => {
    ...
    try {
      const res = await fetch(`${BASE}/subscription/embed/init`, {
        method: 'POST',
      ...
      setMountingElement(false)
    }
  }

  // Mounts the Payment Element once the container div exists (phase === 'payment').
  useEffect(() => {
  ...
  const handleBackToForm = () => {
    try { paymentElementRef.current && paymentElementRef.current.destroy() } catch {}
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      ...
      // No redirect needed (e.g. card didn't require 3DS) — payment is done.
      setPhase('success')
      loadStatus()
    } catch (err) {
            ...
            {phase === 'success' ? (
              <>
                ...
                <div style={{ fontSize: 12, color: '#666', marginBottom: 18 }}>
                  Your subscription status above will update automatically once Stripe confirms the payment.
                </div>
                <Button variation="secondary" size="small" onClick={() => { handleBackToForm(); setPhase('form') }}>
              ...
              <>
                ...
                <div style={{ fontSize: 12, color: '#666', marginBottom: 18 }}>
                  {plan === 'yearly' ? `Yearly plan — $${pricing.yearlyUsd}/yr` : `Monthly plan — $${pricing.monthlyUsd}/mo`}
                  {' · '}{email}
                </div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <Button variation="tertiary" size="small" onClick={handleBackToForm} disabled={paying}>
                    ← Back
                  </Button>
              ...
              <>
                ...
                <div style={{ fontSize: 12, color: '#666', marginBottom: 18 }}>
                  Payment is collected below — you won't be redirected to another site.
                </div>
                ...
                <Button
                  ...
                  isLoading={startingPayment || mountingElement}
                >
                  Continue to payment
                </Button>
              </>
...
export default SellerSubscriptionEmbed
