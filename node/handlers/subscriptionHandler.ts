...
export async function stripeWebhookHandler(ctx: ServiceContext<Clients>) {
  ...
  try {
    if (event.type === 'checkout.session.completed') {
      ...
      // A subscription created without a Checkout Session (e.g. directly via
      // the Stripe API/dashboard) has no prior event to seed name/email/
      // company/phone from, so this can be the first event to populate them —
      // read from the subscription's own metadata (set at creation time) and
      // only fall back to `existing`.
